...
class PoolDatasetService(CRUDService):
    ...
    def validate_encryption_data(self, job, verrors, encryption_dict, schema):
        ...
        msg = None
        if not encryption_dict['key_file']:
            if passphrase_key_format and not encryption_dict['passphrase']:
                msg = 'Please provide a passphrase or a key file with the passphrase'
            elif not encryption_dict['generate_key'] and not passphrase_key_format:
                msg = 'Please provide a key file or select generate_key to automatically generate a key'

        if msg:
            verrors.add(f'{schema}.key_format', msg)
